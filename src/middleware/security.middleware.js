...
const securityMiddleware = async ( req, res, next) => {
    ...
    try {
        ...
        if(decision.isDenied() && decision.reason.isShield()) {
            logger.warn('Shield blocked request', { ip: req.ip, userAgent: req.userAgent, path: req.path });
            
            return res.status(403).json({ error: "Forbidden", message: 'Request blocked by the security policy'})
        }

        if(decision.isDenied() && decision.reason.isRateLimit()) {
            logger.warn('Rate limit exceeded', { ip: req.ip, userAgent: req.userAgent, path: req.path });
